...
for (i in 1:length(fechas)){
  ...
  totesp <- format(sum(plotmap$Fallecidos, na.rm = TRUE),big.mark=",")
  ...
  if (sum(plotmap$Fallecidos, na.rm = TRUE) > 0){
  ...
  if (sum(Can$Fallecidos, na.rm = TRUE) > 0){
  ...
  layoutLayer(title=paste0("COVID19: Fallecidos en España a ", format(Sys.Date(),"%d %b")),
              scale=FALSE,
              frame = FALSE,
